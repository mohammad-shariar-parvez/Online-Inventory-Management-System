        ...
        if ($ins1 == true ) {
            for($q = 0; $q < count($_POST["p_id"]); $q++) {
                ...
                if ($con->query($iquery2) === TRUE) {
                    $mainqty = 0;
                    $manageQty = 0;
                    $SelectQSL= "SELECT * FROM products_info WHERE id = '$p_id';";
                    $selectresult = $con->query($SelectQSL);
                    foreach ($selectresult as $sr) {
                        $mainqty = $sr['quantity'];
                    }
                    $manageQty = $mainqty - $qty;
                    // echo $manageQty;
                    // echo $p_id;
                    $upSQL = "UPDATE products_info SET quantity ='$manageQty' WHERE id ='$p_id'";
